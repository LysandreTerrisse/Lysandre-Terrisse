...
def get_data(path_images, path_labels, n_elements):
	with gzip.open(path_images, "rb") as fd1, gzip.open(path_labels, "rb") as fd2:
		fd1.read(16)
		fd2.read(8)
		images = [np.array(list(fd1.read(28*28)))/255 for _ in range(n_elements)]
		labels = [np.arange(10)==label for label in list(fd2.read(n_elements))]
...
class Neural_network:
	def __init__(self, shape, is_random=False):
		self.shape = shape
		self.W = np.array([np.random.randn(y, x) if is_random else np.empty([y, x]) for x, y in zip([0] + shape[:-1], [0] + shape[1:])], dtype=object)
...
learn(training_data, test_data, [28*28, 30, 10], 1.5, 10)
